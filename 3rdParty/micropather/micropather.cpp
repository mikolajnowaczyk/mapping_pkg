...
long unsigned int PathNodePool::Hash( void* voidval )
{
	/*
...
{
    unsigned key = (unsigned)Hash( _state );
...
void MicroPather::GetNodeNeighbors( PathNode* node, std::vector< NodeCost >* pNodeCost )
{
	...
	else if ( node->cacheIndex < 0 )
	{
		...
		if ( node->numAdjacent > 0 ) {
            ...
            const size_t stateCostVecSize = stateCostVec.size();
